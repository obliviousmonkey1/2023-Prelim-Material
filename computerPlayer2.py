...
class Dastan:
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            SquareIsValid = False
            Choice = 0
            if self._CurrentPlayer.GetName() == 'Player Two':
                print('hello')
                Choice = self._Computer.selectChoice()
                StartSquareReference = self._Computer.choosePiece()
                FinishSquareReference = random.choice(self.__getAllLegalMoves(Choice, StartSquareReference))
                self._Computer.updatePosition(FinishSquareReference)
            ...
            if MoveLegal:
                ...
                print(type(StartSquareReference))
...
class Computer:
    def __init__(self) -> None:
        self.piecePositions = [25,35,45,46,55]
        self.currentPosSelected = ''

    def selectChoice(self):
        return random.randint(1, 5)
    
    def choosePiece(self):
        self.currentPosSelected = random.choice(self.piecePositions)
        return self.currentPosSelected

    def updatePosition(self,f):
        s = []
        for i in self.piecePositions:
            if i == self.currentPosSelected:
                s.append(f)
            s.append(i)
        self.piecePositions = s
