...
class Dastan:
    def __init__(self, R, C, NoOfPieces, NoOfShuffles=1):
        self._Board = []
        self._Players = []
        self._MoveOptionOffer = []
        self._Players.append(Player("Player One", 1, NoOfShuffles))
        self._Players.append(Player("Player Two", -1, NoOfShuffles))
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                Choice = int(input("Choose move option to use from queue (1 to 3), 9 to take the offer, 0 to shuffle queue: "))
                if Choice == 9:
                    self.__UseMoveOptionOffer()
                    self.__DisplayState()
                if Choice == 0:
                    if self._CurrentPlayer.GetNoShuffle() > 0:
                        self.__Shuffle()
                        self._CurrentPlayer.ChangeNumberShuffles(-1)
                        self.__DisplayState()
                    else:
                        print("Can't shuffle no shuffles available")
    ...
    def __Shuffle(self):
        cQueue = self._CurrentPlayer.GetQueue()
        nQueue = []
        for i in range(5):
            r = random.choice(cQueue)
            nQueue.append(r)
            cQueue.pop(cQueue.index(r))
        self._CurrentPlayer.ShuffleQueue(nQueue)
...
class MoveOptionQueue:
    ...
    def ReplaceQueue(self, nQueue):
        self.__Queue = nQueue

class Player:
    def __init__(self, N, D, noOfShuffles):
        self.__Score = 100
        self.__Name = N
        self.__Direction = D
        self.__Queue = MoveOptionQueue()
        self.__NoOfShuffles = noOfShuffles
    ...
    def GetNoShuffle(self):
        return self.__NoOfShuffles

    def ChangeNumberShuffles(self, n):
        self.__NoOfShuffles += n
    
    def ShuffleQueue(self, nQueue):
        self.__Queue.ReplaceQueue(nQueue)
