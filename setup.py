...
def GetSetup():
    try:
        noPieces = int(input('Please enter the number of pieces : '))
        width = int(input('Please input the width of the board : '))
        height = int(input('Please input the height of the board : '))
        if noPieces >= 3 and width >= noPieces +2 and height >= 5:
            return height,width, noPieces 
        else:
            return GetSetup()
    except:
        return GetSetup()
...
def Main():
    height,width, noPieces  = GetSetup()
    ThisGame = Dastan(height, width, noPieces)
    ThisGame.PlayGame()
    print("Goodbye!")
    input()
