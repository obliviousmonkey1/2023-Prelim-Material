...
class Dastan:
    def __init__(self, R, C, NoOfPieces, load):
        if not load:
            self._Board = [] #
            self._Players = [] #
            ...
            players, c, r, moop, moo, board, cP = self.load.startup()
            self._NoOfRows = r
            self._NoOfColumns = c
            self._MoveOptionOfferPosition = moop
            self._MoveOptionOffer = moo
            

            for player in players:
                self._Players.append(Player(player[1], player[2], player[0], player[3], 1))
            
            if cP == 'Player One':
                self._CurrentPlayer = self._Players[0]
                print(self._CurrentPlayer.GetName())
            else:
                self._CurrentPlayer = self._Players[1]

            self._Board = self.rebuildBoard(board)
            print(self._Board)
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                Choice = int(input("Choose move option to use from queue (1 to 3) or 9 to take the offer or 0 to save: "))
                if Choice == 9:
                    ...
                    a = SaveLoad()
                    board = self.getBoard()
                    a.save(self._Players, self._NoOfColumns, self._NoOfRows,  self._MoveOptionOfferPosition,  self._MoveOptionOffer, board, self._CurrentPlayer.GetName())
                    self.__DisplayState()
    ...
    def rebuildBoard(self, board):
        ls = []
        for i in board:
            if i[-1] == 'K' or i[-1] == 'k':
                if i[0][-1] == '1' or i[0][-1] == '2':
                    ls.append(Square(Piece(i[0][0], Player(i[0][1][1], i[0][1][2], i[0][1][0], i[0][1][3], 1), i[0][2], i[0][3]), i[1], i[2], 1))
                else:
                    ls.append(Kotla(Player(i[1][1], i[1][2], i[1][0], i[1][3], 1), i[-1]))
    ...
    def getBoard(self):
        ...
        for i in self._Board:
            if i.GetSymbol() == 'K' or i.GetSymbol() == 'k':
                ls.append([i.GetPIS(), i.gBT(), i.GetSymbol()])
            else:
                ls.append([i.GetPIS(), i.GetBelongsTo(), i.GetSymbol()])
        return ls 
...
class Piece:
    ...
    def gBT(self):
        return [self._BelongsTo.GetScore(),self._BelongsTo.GetName(),self._BelongsTo.GetDirection(),self._BelongsTo.GetQueueP()]

    def GetPointsIfCaptured(self):
        return self._PointsIfCaptured

class Square:
    def __init__(self, pIS = None, bT= None, s = " ", load = 0):
        if load:
            self._PieceInSquare = pIS
            self._Symbol = s
            self._BelongsTo = bT
        else:
            self._PieceInSquare = pIS
    ...
    def gBT(self):
        try:
            ...
            self._BelongsTo = Player(self._BelongsTo[1],self._BelongsTo[2],self._BelongsTo[0],self._BelongsTo[3],1)
            return [self._BelongsTo.GetScore(),self._BelongsTo.GetName(),self._BelongsTo.GetDirection(),self._BelongsTo.GetQueueP()]

    def GetPIS(self):
        try:
            return [self._PieceInSquare.GetTypeOfPiece(),self._PieceInSquare.gBT(),self._PieceInSquare.GetPointsIfCaptured(),self._PieceInSquare.GetSymbol()]
        except:
            return []
...
class MoveOption:
    def __init__(self, N, pM= [], load = 0):
        if load:
            self._PossibleMoves = [Move(i[0], i[1]) for i in pM]
        else:
            self._PossibleMoves = pM
        self._Name = N
...
class Move:
    ...
    def GetMove(self):
        return [self._RowChange, self._ColumnChange]
...
class MoveOptionQueue:
    def __init__(self, queue = [], load = 0):
        if load:
             self.__Queue = [MoveOption(i[0], i[1], 1) for i in queue]
        else:
            self.__Queue = queue
    ...
    def getQueue(self):
        ...
        for i in self.__Queue:
            if i.GetName() in names:
                break
            c.append([i.GetName(), i.GetPossibleMoves()])
...
class Player:
    def __init__(self, N, D, score = 100, queue = MoveOptionQueue(), load=0):
        if load:
            self.__Queue = MoveOptionQueue(queue, 1)
        else:
            self.__Queue = queue
        self.__Score = score
        self.__Name = N
    ...
    def SameAs(self, APlayer):
        try:
            ...
            APlayer = Player(APlayer[1], APlayer[2], APlayer[0], APlayer[3], 1)
            if APlayer is None:
                return False
            elif APlayer.GetName() == self.__Name:
                return True
...
class SaveLoad:
    ...
    def find(self):
        ...
        for i in list:
            if '.das' in i:
                ...
                a += 1
                ls.append(i)
        saveIndex = int(input('Which save do you want to load : '))
        self.load(ls[saveIndex])
    ...
    def startup(self):
        ...
        return ls, self.data['general']['noOfCollumns'], self.data['general']['noOfRows'], self.data['general']['MOOP'], self.data['general']['MOO'], self.data['board'], self.data['general']['CP'] 
    

    def save(self, players, noC, noR, moop, moo, board, cp):
        pl = [players[0].GetScore(), players[0].GetName(), players[0].GetDirection(), players[0].GetQueueP(), players[1].GetScore(), players[1].GetName(), players[1].GetDirection(), players[1].GetQueueP()]
        saveName = input('enter save name : ')
        x = {
        "general":
            {"noOfCollumns" : noC, "noOfRows" : noR, 'MOOP' : moop, 'MOO': moo, 'CP' : cp},
        "players": [
            {"score": pl[0], "name":  f'{pl[1]}', "direction": pl[2], "queue": pl[3]},
            {"score": pl[4], "name": f'{pl[5]}', "direction": pl[6], "queue": pl[7]}
        ],
        "board" : board
        }

        with open(f'{saveName}.das', 'w') as f:
            json.dump(x, f)
...
def Main():
    load = int(input('Would you like to load a previous game ? (0/1) : '))
    ThisGame = Dastan(6, 6, 4, load)
    ThisGame.PlayGame()
    print("Goodbye!")
    input()
