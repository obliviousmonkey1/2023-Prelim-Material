...
class Dastan:
    ...
    def __CalculatePieceCapturePoints(self, FinishSquareReference):
        if self._Board[self.__GetIndexOfSquare(FinishSquareReference)].GetPieceInSquare() is not None:
            if self._CurrentPlayer.SameAs(self._Players[0]):
                self._Players[1].ChangeNumberOfCapturedPieces('+',1)
            else:
                self._Players[0].ChangeNumberOfCapturedPieces('+',1)
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                print(f'Pieces lost {self._CurrentPlayer.GetNumberOfCapturedPieces()}')
                ...
                if Choice == 9:
                    ...
                    if self._CurrentPlayer.GetNumberOfCapturedPieces() > 0 and self._CurrentPlayer.GetBringBack() == 1:
                        self._CurrentPlayer.ChangeBringBack()
                        self.__CapturedPiece()
                        self.__DisplayState()
    ...
    def __CapturedPiece(self):
        
        self._CurrentPlayer.ChangeNumberOfCapturedPieces('-',1)
        ...
        self._Board[self.__GetIndexOfSquare(FinishSquareReference)].SetPiece(self._Board[self.__GetIndexOfSquare(54)].PieceType())
...
class Player:
    def __init__(self, N, D):
        ...
        self.__NumberOfCapturedPieces = 0
        self.__bringBack = 1
    ...
    def GetBringBack(self):
        return self.__bringBack

    def GetNumberOfCapturedPieces(self):
        return  self.__NumberOfCapturedPieces
    ...
    def ChangeBringBack(self):
        self.__bringBack -= 1

    def ChangeNumberOfCapturedPieces(self, change, amount):
        if change == '+':
            self.__NumberOfCapturedPieces += amount
        elif change == '-':
            self.__NumberOfCapturedPieces -= amount
