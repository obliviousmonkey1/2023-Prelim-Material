...
class Dastan:
    ...
    def __GetSquareReference(self, Description):
        try:
            ...
            print("must be and integer")
        return self.__GetSquareReference(Description)
    ...
    def __CreateMoveOptions(self):
        for move in ['ryott','chowkidar','cuirassier','faujdar','jazair']:
            for i in range(0,1):
                self._Players[i].AddToMoveOptionQueue(self.__CreateMoveOption(move, (1-(i+i))))
...
class MoveOptionQueue:
    ...
    def Cycle(self):
        self.MoveItemToBack(0)
    
    def Roulette(self):
        for i in range(random.randint(1,(len(self.__Queue)-1))):
            self.MoveItemToBack(0)
