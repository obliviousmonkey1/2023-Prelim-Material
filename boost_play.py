...
class Dastan:
    ...
    def PlayGame(self):
        GameOver = False
        print(self._CurrentPlayer.GetRyott())
        while not GameOver:
            self.__DisplayState()
            SquareIsValid = False
            Choice = 0
            ...
            while Choice < 1 or Choice > 3:
                if usingBoost:
                    Choice = int(input(f"Choose move option to use from queue (1 to 3) or 9 to take the offer : "))
                else:
                    Choice = int(input(f"Choose move option to use from queue (1 to 3) or 9 to take the offer or 10 to use boost [Boost available {self._CurrentPlayer.isBoostAvailable()}]: "))
                if Choice == 9:
                    self.__UseMoveOptionOffer()
                    self.__DisplayState()
                elif Choice == 10 and self._CurrentPlayer.isBoostAvailable():
                    usingBoost = True
                    self._CurrentPlayer.SetBoostUsed()
            ...
            if usingBoost:    
                Choice = self._CurrentPlayer.GetRyott()
                SquareIsValid = False
                while not SquareIsValid:
                    StartSquareReference = self.__GetSquareReference("containing the piece [ryott] to move")
                    SquareIsValid = self.__CheckSquareIsValid(StartSquareReference, True)
                SquareIsValid = False
                while not SquareIsValid:
                    FinishSquareReference = self.__GetSquareReference("to move to")
...
class Player:
    def __init__(self, N, D):
        ...
        self.__Boost_Available = True

    def GetRyott(self):
        return self.__Queue.GetRyottPos()
    ...
    def SetBoostUsed(self):
        self.__Boost_Available = False
