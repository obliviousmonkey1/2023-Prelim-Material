...
class Dastan:
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                Choice = int(input("Choose move option to use from queue (1 to 3) or 9 to take the offer, 7 to Mirzaring : "))
                if Choice == 9:
                    self.__UseMoveOptionOffer()
                    self.__DisplayState()
                if Choice == 7:
                    if self._CurrentPlayer.GetCanMirzaring():
                        self.__Mirzaring()
                        self._CurrentPlayer.ChangeCanMirzaring()
                        self.__DisplayState()
    ...
    def __Mirzaring(self):
        for r in range(self._NoOfRows+1):
            for c in range(self._NoOfColumns):
                StartSquare = int(str(r) + str(c))
                if self.__CheckSquareInBounds(StartSquare):
                    PieceInSquare = self._Board[self.__GetIndexOfSquare(StartSquare)].GetPieceInSquare()
                    if PieceInSquare is not None:

                        if self._CurrentPlayer.SameAs(PieceInSquare.GetBelongsTo()):
                            if PieceInSquare.GetTypeOfPiece() == 'mirza':
                                a = True
                                while a:
                                    c -= 1
                                    SquareReference = int(str(r) + str(c))
                                    if not self.__CheckSquareInBounds(SquareReference):
                                        EndSquare = int(str(r) + str(c+1))
                                        a = False
                                    ...
                                    if PieceInSquare is not None:
                                        ...
                                        if self._CurrentPlayer.SameAs(PieceInSquare.GetBelongsTo()):
                                            ...
                                            EndSquare = int(str(r) + str(c))
                                            a = False
                                self.__UpdateBoard(StartSquare, EndSquare)
...
class Player:
    ...
    def GetCanMirzaring(self):
        return self.__CanMirzaring
    ...
    def ChangeCanMirzaring(self):
        self.__CanMirzaring = False
