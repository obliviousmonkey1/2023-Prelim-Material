...
class Dastan:
    ...
    def __CheckBarbarianMove(self, SquareReference, StartSquare):
        if not self.__CheckSquareInBounds(SquareReference):
            return False
        PieceInSquare = self._Board[self.__GetIndexOfSquare(SquareReference)].GetPieceInSquare()
        ...
        if not self._CurrentPlayer.SameAs(PieceInSquare.GetBelongsTo()):
            return True
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            SquareIsValid = False
            if self.__Queue.GetMoveOptionInPosition(Choice - 1) == 'barbarian':
                i = 1
                for c in range(self._NoOfColumns):
                    finishSquareRef = int(str(StartSquareReference[0]) + str(StartSquareReference[1]+i))
                    squareIsValid = self.__CheckBarbarianMove(finishSquareRef, False)
                    # if reaches the end of the board or infront of a teammate 
                    if not squareIsValid:
                        return int(str(StartSquareReference[0]) + str(StartSquareReference[1]+(i-1)))
                    elif squareIsValid:
                        return finishSquareRef
                    i+=1
            else:
                while not SquareIsValid:
    ...
    def __CreateMoveOptionOffer(self):
        ...
        self._MoveOptionOffer.append('barbarian')
    ...
    def __CreateBarbarianMoveOption(self, Direction):
        NewMoveOption = MoveOption("barbarian")
        NewMove = Move(1 * Direction, 1 * Direction)
        NewMoveOption.AddToPossibleMoves(NewMove)
        NewMove = Move(1 * Direction, -1 * Direction)
        NewMoveOption.AddToPossibleMoves(NewMove)
