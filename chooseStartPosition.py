...
class Dastan:
    def __init__(self, R, C, NoOfPieces):
        ...
        # EDIT START 
        p1Pos, p2Pos = self.__GetStartingPosition(R,C)
        self.__CreateBoard(p1Pos, p2Pos)
        self.__CreatePieces(NoOfPieces, p1Pos, p2Pos)
        #EDIT END
    ...
    # EDIT START 
    def __ValidateStartPosition(self, pos, player, R, C):
        if player == 1:
            if int(pos[0]) == 1: 
                if (int(pos[1]) >= 1 and int(pos[1]) <= C):
                    return True
        elif player == 2:
            if int(pos[0]) == R and (int(pos[1]) >= 1 and int(pos[1]) <= C):
                return True
        return False

    def __GetStartingPosition(self, R, C):
        p1pos = int(input('RC : '))
        while not self.__ValidateStartPosition(str(p1pos), 1, R, C):
            p1pos = int(input('RC : '))
        
        p2pos = int(input('RC : '))
        while not self.__ValidateStartPosition(str(p2pos), 2, R, C):
            p2pos = int(input('RC : '))
        
        return str(p1pos), str(p2pos) 
    # EDIT END
    ...
    def __GetIndexOfSquare(self, SquareReference, type = 0):
        # EDIT START
        if type == 1:
            Row = int(SquareReference[0])
            Col = int(SquareReference[1])
        else:
    ...
    def __UseMoveOptionOffer(self):
        ...
        while ReplaceChoice < 0 or ReplaceChoice > 5:
            ReplaceChoice = int(input("Choose the move option from your queue to replace (1 to 5): "))
        # EDIT END
    ...
    def __CreateBoard(self, p1pos, p2pos):
        for Row in range(1, self._NoOfRows + 1):
            for Column in range(1, self._NoOfColumns + 1):
                # EDIT START
                if Row == int(p1pos[0]) and Column == int(p1pos[1]):
                    S = Kotla(self._Players[0], "K")
                elif Row == int(p2pos[0]) and Column == int(p2pos[1]):
    ...
    def __CreatePieces(self, NoOfPieces, p1pos, p2pos):
        ...
        # EDIT START 
        self._Board[self.__GetIndexOfSquare(p1pos,1)].SetPiece(CurrentPiece)
        # EDIT END
        for Count in range(1, NoOfPieces + 1):
        ...
        # EDIT START
        self._Board[self.__GetIndexOfSquare(p2pos, 1)].SetPiece(CurrentPiece)
        # EDIT END
